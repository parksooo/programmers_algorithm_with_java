...
public class ReturnArmyCamp_132266 {
    int[] cost;
    List<Integer>[] graph;
    ...
    private void bfs(int start) {
        ...
        while (!q.isEmpty()) {
            int currSpot = q.poll();
            int len = graph[currSpot].size();

            for (int i = 0; i < len; i++) {
                int nextSpot = graph[currSpot].get(i);
                if (cost[nextSpot] == -1) {
                    cost[nextSpot] = cost[currSpot] + 1;
                    q.add(nextSpot);
                }
            }
        }
    ...
    public static void main(String[] args) {
        System.out.println(
                Arrays.toString(new ReturnArmyCamp_132266().solution(
                        5,
